...
interface SyncCmd {
    ...
    companion object {
        ...
        val filesStartPposition = commndLength + 1
        ...
        fun makeCommand(cmd: String, lineNumber: Int): SyncCmd =
            if (cmd.length < 15)
                ...
                "  mk <folder> " -> MkDirCmd(lineNumber, cmd.substring(filesStartPposition))
                " del <folder> " -> DelCmd(lineNumber, cmd.substring(filesStartPposition))
                " del < file > " -> DelCmd(lineNumber, cmd.substring(filesStartPposition))
                "copy < file > " -> cmd.substring(filesStartPposition).split(" --> ").let{ CopyCmd(lineNumber, it[0], it[1]) }
